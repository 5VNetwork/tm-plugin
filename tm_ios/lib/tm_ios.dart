...
import 'package:tm_ios/src/messages.g.dart';
import 'package:tm_platform_interface/tm_platform_interface.dart';
import 'package:flutter/services.dart';

class TmIos extends TmPlatform {
  static void registerWith() {
    print('TmIos registerWith');
    TmPlatform.instance = TmIos();
  }
  ...
  final BehaviorSubject<TmStatusChange> _statusStream =
      BehaviorSubject<TmStatusChange>.seeded(
          TmStatusChange(status: TmStatus.unknown));
  ...
  Future<void> start({
    ...
    return _tunnelApi.start(config.writeToBuffer(),
        getTunnelConfig(config.tun, defaultNicSupport6));
  }
...
TunnelConfig getTunnelConfig(TunConfig tunConfig, bool? defaultNicSupport6) {
  final result = TunnelConfig(
    mtu: tunConfig.device.mtu == 0 ? 1500 : tunConfig.device.mtu,
    useFd: true,
    tun46Setting: tunConfig.tun46Setting.value,
